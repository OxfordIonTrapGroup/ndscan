...
class ArgumentEditor(QtWidgets.QTreeWidget):
    def __init__(self, manager, dock, expurl):
        ...
        set_resize_mode(1, QtWidgets.QHeaderView.ResizeToContents)
        set_resize_mode(2, QtWidgets.QHeaderView.Stretch)
        set_resize_mode(3, QtWidgets.QHeaderView.ResizeToContents)
        ...
        if not ndscan_params:
            ...
            for path in ndscan_params["always_shown_params"]:
                ...
                self._add_param_item(path, schema, True)

            for name, argument in vanilla_args.items():
                self._add_vanilla_argument_item(name, argument)

            self.override_separator = self._add_line_separator()

            self.add_override_item = QtWidgets.QTreeWidgetItem()
            self.addTopLevelItem(self.add_override_item)
    ...
    def _add_param_item(self, path, schema, show_always):
        widgets = dict()
        ...
        def add_item(widget_item):
            ...
            if not group:
                ...
                self._get_group_widget(group).addChild(widget_item)

        fqn = self.shortened_fqns[schema["fqn"]]
        id_string = fqn + "@" + "/".join(path.split("/")[:-1]) + "/"
        ...
        font = main_item.font(1)
        font.setBold(True)
        main_item.setFont(1, font)

        buttons_left = LayoutWidget()
        ...
        buttons_left.addWidget(remove_override)
        self.setItemWidget(main_item, 0, buttons_left)
        ...
        self.setItemWidget(main_item, 2, entry)

        buttons_right = LayoutWidget()
        ...
        buttons_right.addWidget(reset_default)
        self.setItemWidget(main_item, 3, buttons_right)
    ...
    def _add_vanilla_argument_item(self, name, argument):
        ...
        font = widget_item.font(1)
        font.setBold(True)
        widget_item.setFont(1, font)

        if argument["group"] is None:
            self.addTopLevelItem(widget_item)
        else:
            self._get_group_widget(argument["group"]).addChild(widget_item)
        ...
        buttons.addWidget(recompute_argument, col=0)

        buttons.layout.setColumnStretch(0, 0)
        buttons.layout.setColumnStretch(1, 1)

        self.setItemWidget(widget_item, 3, buttons)
    ...
    def _set_override_line_idle(self):
        layout = LayoutWidget()
        add_override_button = QtWidgets.QToolButton()
        add_override_button.setIcon(self.add_override_icon)
        layout.addWidget(add_override_button)
        layout.layout.setColumnStretch(0, 0)
        layout.layout.setColumnStretch(1, 1)
        self.setItemWidget(self.add_override_item, 0, layout)

    def _get_group_widget(self, name):
